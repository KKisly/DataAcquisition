...
void loop()
{
   ...
   readingsA[indexA] = (readingsA[indexA]-512)*4.9/1024/0.013-0.014;//Data processing:510-raw data from analogRead when the input is 0; 5-5v; the first 0.04-0.04V/A(sensitivity); the second 0.04-offset val;
   totalA = totalA + readingsA[indexA];       
   indexA = indexA + 1;                    
...
// Second motor - B
   ...
   readingsB[indexB] = (readingsB[indexB]-511)*4.9/1024/0.013-0.014;//Data processing:510-raw data from analogRead when the input is 0; 5-5v; the first 0.04-0.04V/A(sensitivity); the second 0.04-offset val;
   totalB = totalB + readingsB[indexB];       
   indexB = indexB + 1;                    
...
// RPM Measurements ********************************************************************************************
{
  if (millis() - lastmillis >= 450){ //Uptade every one second, this will be equal to reading frecuency (Hz).
...
detachInterrupt(digitalPinToInterrupt(20));//Disable interrupt when calculating
 rpm = (half_revolutions * 120)/13; // Convert frecuency to RPM, note: this works for one interruption per full rotation. For two interrups per full rotation use half_revolutions * 30.
 rpm2 = (half_revolutions2 * 120)/13; // Convert frecuency to RPM, note: this works for one interruption per full rotation. For two interrups per full rotation use half_revolutions * 30.
